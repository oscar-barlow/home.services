...
firewall-remove:
	...
	sudo ebtables -D FORWARD -p 0x0800 --ip-src 192.168.1.224/27 --ip-dst 192.168.1.192/27 -j DROP 2>/dev/null || echo "ebtables preprod→prod rule not found"
	sudo ebtables -D FORWARD -p 0x0800 --ip-src 192.168.1.192/27 --ip-dst 192.168.1.224/27 -j DROP 2>/dev/null || echo "ebtables prod→preprod rule not found"
