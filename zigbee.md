...
3. Deploy with `make env-up ENV=prod` from the N100. Mosquitto and Zigbee2MQTT
   come up as Swarm services; ser2net is a standalone container that deploys
   onto the Pi over SSH via a Docker context (`SER2NET_DOCKER_CONTEXT=pi`), so
   the one command brings up the whole stack. This needs a one-time `pi` context
   on the deploy machine — see [ser2net/README.md](ser2net/README.md).
